...
SIGN_V, SIGN_H = HB + 0.2, 8.0
CAP_PROF = [(0.0, 0.0), (1.2, 0.0), (1.2, 0.6), (1.6, 0.6), (1.6, 1.4), (3.2, 1.4), (3.2, 2.0), (3.4, 2.2), (3.6, 2.6),
            (3.6, 3.2), (0.0, 3.2)]
CAP_V = SIGN_V + SIGN_H + 0.4
FF = CAP_V + 3.2          # false-front top
CREST = (12.0, 36.0, 4.4)
...
def _openings():
    ...
    add(8.0, 0, 0.0, SF.door_batwing(8.0, 22.0, text="SALOON"), "saloon-door", "door")
    big = SF.window_commercial(9.0, 18.0, rise=0, lites=(1, 3), rows=(1, 1), sill=1.2)
    add(20.0, 0, 6.0, big, "saloon-window")
    add(34.0, 0, 0.0, SF.door_commercial(8.0, 21.0, transom=3.4, leaf="french", tstyle="text:HOTEL", head=None, leaves=2),
        "hotel-door", "door")
    add(44.0, 0, 6.0, SF.window_commercial(6.4, 18.0, rise=0, lites=(1, 3), rows=(1, 1), sill=1.2), "lobby-window")
    bal = SF.door_commercial(7.0, 21.0, transom=0.0, leaf="french", tstyle="text:HOTEL", head=None, leaves=2)
    for x in (8.0, 24.0, 40.0):
        add(x, 0, V2, bal, f"balcony-{x:.0f}", "door")
    w3 = SF.window_commercial(5.6, 15.0, rise=0, lites=(1, 3), rows=(1, 1), sill=1.0, head="drip")
    for x in (5.0, 14.0, 23.0, 32.0, 41.0):
        add(x, 0, V3 + 4.0, w3, f"F{x:.0f}-3")
    side = SF.window_commercial(6.4, 16.0, rise=0, lites=(1, 3), rows=(1, 1), sill=1.0, head="drip")
    for y in (14.0, 30.0, 46.0):
        for v in (V2 + 4.0, V3 + 4.0):
            add(W, y, v, side, f"E{y:.0f}-{v:.0f}")
            add(0, y, v, side, f"W{y:.0f}-{v:.0f}")
    add(W - 12.0, D, 0.0, SF.door_commercial(7.0, 21.0, transom=3.0, leaf="french", tstyle="text:HOTEL", head=None),
        "back-door", "door")
    for x in (14.0, 34.0):
        add(W - x, D, V2 + 4.0, side, f"N{x:.0f}-2")
        add(W - x, D, V3 + 4.0, side, f"N{x:.0f}-3")
    return L


OPENINGS = _openings()
_VS, VS_H = SF.vertical_sign("HOTEL", cap=3.0)
VS_V = V3 - 2.0
APPLIED = [("SIGN", 2.0, SIGN_V, W - 4.0, SIGN_H), ("CAP", 0.0, CAP_V, W, 3.2),
           ("CREST", CREST[0], FF, CREST[1] - CREST[0], CREST[2])] + \
          [(f"BRK{k}", u - 0.8, V2 - RH - 5.0, 1.6, 5.0) for k, u in enumerate(BRACKETS)] + \
...
def build(kit=None):
    ...
    for k, u in enumerate(BRACKETS):
        br = SF.console(5.0, BAL_D - 0.4, 1.4, 0.0, 0.0, style="knee")
        A_ = frame_at(u, v_brk)
    ...
    sign = SF.sign_band(W - 4.0, SIGN_H, "PALACE HOTEL", cap=3.4, font="roman", board=1.0, frame=0.8, relief=0.4)
    As = frame_at(2.0, SIGN_V)
    kit.add("SIGN", "Sign", sign.transform(As), P=inv34(As), group="front", render=_gilt(sign, As, 1.0, "Sign"))
    cap = SF.cornice_cap(W, CAP_PROF, dentils=dict(v=0.6, h=0.8, d=1.4, tooth=0.6, gap=0.6))
    ...
    crest = box([0, 0, 0], [cw, CREST[2], 0.8]) + box([-0.3, CREST[2] - 1.0, 0.0], [cw + 0.3, CREST[2], 1.4]) + \
        ext(SF.text_cs("1883", 2.2, "roman", grow=0.1).translate((cw / 2, 0.6)), 0.79, 1.2)
