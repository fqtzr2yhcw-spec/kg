...
def window_insert(w, h, rise=None, style="crest", lites=(1, 1), casing=1.1, bare=False, ends=0.9, sill_ext=0.6,
    ...
    if upper == "ytracery":          # Y tracery: a bar up the upper sash forking into two curved bars (the Wisteria)
        top_ = spring if rise > 0 else h
        vf = mr + (top_ - mr) * 0.42
        bars.append(rect(-RIB / 2, mr, RIB / 2, vf + 0.2))
        for sgn in (-1, 1):
...
def _leaf(style, u, lw, dh, hinge_left):
    ...
    if style == "arch_panels":
    ...
    elif style == "ellipse":              # an upright elliptical light over a panel with a raised square
        cx = (pu0 + pu1) / 2
        rx = (pu1 - pu0) / 2 - 0.35
        vlo, vhi = dh * 0.45, top - st - 0.4
        cy, ry = (vlo + vhi) / 2, (vhi - vlo) / 2
        gl = poly([(cx + rx * math.cos(a), cy + ry * math.sin(a)) for a in np.linspace(0, 2 * math.pi, 40, endpoint=False)])
        glass = gl
        parts.append(ext(gl.offset(0.45, JoinType.Round) - gl, -0.8, -0.6))
        lo = rect(pu0, 1.3, pu1, dh * 0.45 - 1.0)
        panel(lo)
        b = lo.bounds()
        s_ = min(0.8, (b[2] - b[0]) * 0.25)
        parts.append(ext(rect((b[0] + b[2]) / 2 - s_, (b[1] + b[3]) / 2 - s_, (b[0] + b[2]) / 2 + s_, (b[1] + b[3]) / 2 + s_),
                         -0.41, -0.2))
        ...
        cx = (pu0 + pu1) / 2
        hw = (pu1 - pu0) / 2
        vlo, vhi = dh * 0.47, top - st - 0.3
        q = min(hw * 0.8, (vhi - vlo) * 0.2)
        gl = poly([(cx, vlo), (cx + hw, vlo + q), (cx + hw, vhi - q), (cx, vhi), (cx - hw, vhi - q), (cx - hw, vlo + q)])
        glass = gl
        parts.append(ext(gl.offset(0.45, JoinType.Miter, 4.0) - gl, -0.8, -0.6))
        lo = rect(pu0, 1.3, pu1, dh * 0.47 - 1.0)
        panel(lo)
        b = lo.bounds()
        hh = (b[3] - b[1]) / 2
        for k in range(2):
            v0_ = b[1] + 0.6 + k * hh
            parts.append(ext(rect(b[0] + 0.6, v0_, b[2] - 0.6, v0_ + hh - 1.2), -0.41, -0.2))
    elif style == "trefoil":              # a light with a trefoil (three-lobed) head over a raised panel (the Camellia)
        gw = pu1 - pu0
        cx = (pu0 + pu1) / 2
        rs, rc = gw * 0.26, gw * 0.3
        vh = top - st - rc * 1.9
        gl = cs_union([rect(pu0, dh * 0.48, pu1, vh), circle((cx - gw / 2 + rs, vh), rs, 20),
                       circle((cx + gw / 2 - rs, vh), rs, 20), circle((cx, vh + rc * 0.85), rc, 24)])
        glass = gl
        parts.append(ext(gl.offset(0.45, JoinType.Round) - gl, -0.8, -0.6))
        panel(rect(pu0, 1.3, pu1, dh * 0.48 - 1.0))
    elif style == "oval":
...
def _ornate_door_sash(w, h, leaves, transom, leaf="arched", tstyle="sunburst"):
    ...
    if transom > 0:
        ...
        if tstyle == "sunburst":
        ...
        elif tstyle == "star":                   # a six-pointed star of bars in the middle of the light
            cy = (tb[1] + tb[3]) / 2
            r = min((tb[3] - tb[1]) / 2 - 0.2, 1.6)
            tri = lambda a0: poly([(r * math.cos(a0 + k * 2 * math.pi / 3), cy + r * math.sin(a0 + k * 2 * math.pi / 3)) for k in range(3)])
            star = [tri(math.pi / 2), tri(-math.pi / 2)]
            pat = cs_union([t - t.offset(-RIB, JoinType.Miter, 4.0) for t in star]) + \
                rect(tb[0] - 1, cy - RIB / 2, -r * 0.85, cy + RIB / 2) + rect(r * 0.85, cy - RIB / 2, tb[2] + 1, cy + RIB / 2)
            ...
            dia = poly([(0.0, cy - r), (r * 1.3, cy), (0.0, cy + r), (-r * 1.3, cy)])
            pat = (dia - dia.offset(-RIB, JoinType.Miter, 4.0)) + rect(tb[0] - 1, cy - RIB / 2, -r * 1.2, cy + RIB / 2) + \
                rect(r * 1.2, cy - RIB / 2, tb[2] + 1, cy + RIB / 2)
        elif tstyle == "fret":                   # a square-wave fret bar running across the light
            n = max(3, int((tb[2] - tb[0]) / 1.6))
            p = (tb[2] - tb[0]) / n
            y0, y1 = tb[1] + 0.6, tb[3] - 0.6
            pts = [(tb[0] - 0.5, y0)]
            for k in range(n):
                x = tb[0] + p * k
                pts += [(x + p * 0.25, y0), (x + p * 0.25, y1), (x + p * 0.75, y1), (x + p * 0.75, y0)]
            pts.append((tb[2] + 0.5, y0))
            pat = stroke(pts, RIB, caps=False)
        elif tstyle == "leaded":
